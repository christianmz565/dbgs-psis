/**
 * @file Parser.cpp
 * @brief Implementation of the simple pipe syntax SQL parser.
 */
...
ParsedCommand Parser::parseCommand(const std::string& command) {
    ...
    // Split by pipe to get command parts
    std::vector<std::string> parts = split(command, '|');
    if (parts.empty()) {
        return cmd;
    }
    
    // Parse first part to determine command type
    ...
    if (upperFirst.find("CREATE TABLE") == 0) {
        ...
        // Parse column definitions from second part
        if (parts.size() > 1) {
        ...
        // Extract table name: "INSERT INTO tablename" or "INSERT tablename"
        std::vector<std::string> tokens = split(firstPart, ' ');
        if (tokens.size() >= 2) {
        ...
        // Parse "SELECT * FROM tablename" or "SELECT FROM tablename"
        std::vector<std::string> tokens = split(firstPart, ' ');
        ...
        if (parts.size() > 1) {
            ...
            if (toUpper(wherePart).find("WHERE") == 0) {
                ...
                if (whereTokens.size() >= 4) { // WHERE column = value
        ...
        // Parse "CREATE INDEX ON tablename"
        std::vector<std::string> tokens = split(firstPart, ' ');
        for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        // Parse column and degree from second part
        if (parts.size() > 1) {
        ...
        // Parse "CREATE INVERTED INDEX ON tablename"
        std::vector<std::string> tokens = split(firstPart, ' ');
        for (size_t i = 0; i < tokens.size(); ++i) {
            if (toUpper(tokens[i]) == "ON" && i + 1 < tokens.size()) {
                cmd.tableName = trim(tokens[i + 1]);
...
bool Parser::executeCreateTable(const ParsedCommand& cmd) {
    ...
    if (tables_.find(cmd.tableName) != tables_.end()) {
        std::cout << "Error: Table '" << cmd.tableName << "' already exists" << std::endl;
        return false;
    }
    ...
    for (const std::string& colDef : cmd.columns) {
        ...
        if (colonPos == std::string::npos) {
            std::cout << "Error: Invalid column definition '" << colDef << "'" << std::endl;
            return false;
        }
    ...
    try {
        ...
        std::cout << "Table '" << cmd.tableName << "' created successfully" << std::endl;
        return true;
    } catch (const std::exception& e) {
...
bool Parser::executeInsert(const ParsedCommand& cmd) {
    ...
    if (!table) {
        std::cout << "Error: Table '" << cmd.tableName << "' not found" << std::endl;
        return false;
    }
    
    // We need to get the table schema to parse values correctly
    // For simplicity, we'll assume values are provided in schema order
    std::vector<Cell> row;
    
    // This is a simplified implementation - in a real parser you'd need
    // to get the table schema and match values to column types
...
bool Parser::executeSelect(const ParsedCommand& cmd) {
    ...
    if (cmd.whereColumn.empty()) {
        ...
        // Search by index if available
        try {
            Cell searchValue = parseValue(cmd.whereValue, DataType::STRING); // Default to string
            std::vector<int> rowIndices = table->searchByIndex(cmd.whereColumn, searchValue);
...
bool Parser::executeCreateIndex(const ParsedCommand& cmd) {
    ...
    if (cmd.indexColumn.empty()) {
        std::cout << "Error: Column name is required for index creation" << std::endl;
        return false;
    }
...
bool Parser::executeCreateInvertedIndex(const ParsedCommand& cmd) {
    ...
    if (cmd.indexColumn.empty()) {
        std::cout << "Error: Column name is required for inverted index creation" << std::endl;
        return false;
    }
    
    try {
        table->createInvertedIndex(cmd.indexColumn);
        std::cout << "Inverted index created on column '" << cmd.indexColumn << "'" << std::endl;
        return true;
    } catch (const std::exception& e) {
